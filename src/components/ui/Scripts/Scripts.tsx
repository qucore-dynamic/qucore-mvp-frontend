...
const Scripts = () => {
  return (
    <div className='Scripts bg-(--bl) border-r border-(--border) min-w-27.5 w-[29.25vw] h-screen flex flex-col justify-start items-center overflow-y-auto overflow-x-hidden'>
      <div className='Scripts_content flex flex-col justify-start items-center gap-12.5 py-12.5'>
        {scripts.map((script) => (
          <button
            key={script.title}
            title={script.descr}
            className='Scripts_btn max-w-90 mx-12.5 px-6.25 py-5 self-stretch text-[clamp(0.625rem,1.5vw,1.25rem)] rounded-[20px] shrink-0 text-center cursor-pointer'
          >
            {script.title}
          </button>
        ))}
